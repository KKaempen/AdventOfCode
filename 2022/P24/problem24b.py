...
total_dist = get_man_dist(start_pos, end_pos)
start = (0 + 3 * total_dist, 0, start_pos, 0, 0)
frontier = [start]
visited = [[[[False for i in range(width)] for j in range(height + 2)] for k in range(cycle_length)] for l in range(3)]
while len(frontier) > 0:
    _, s, p, c, v = heappop(frontier)
    y, x = p
    if p == end_pos and v == 2:
        print(s)
        break
    if (p == end_pos and v == 0) or (p == start_pos and v == 1):
        v += 1
    if visited[v][c][y][x - 1]:
        continue
    visited[v][c][y][x - 1] = True
    c = (c + 1) % cycle_length
    ...
    for a in actions:
        if not storm_poss[c][a[0]][a[1] - 1]:
            ...
            dist += total_dist * (2 - v)
            heappush(frontier, (s + 1 + dist, s + 1, a, c, v))
