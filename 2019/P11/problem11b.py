...
colors = run_intcode(data)
colors = {x: '#' if colors[x] == 1 else ' ' for x in colors}
points = [x for x in colors]
min_x = min([x[0] for x in points])
max_x = max([x[0] for x in points])
min_y = min([x[1] for x in points])
max_y = max([x[1] for x in points])
points = [(x[0] - min_x, x[1] - min_y) for x in points]
for i in range(max_y, min_y - 1, -1):
    row = []
    for j in range(min_x, max_x + 1):
        if (j, i) in colors:
            row.append(colors[(j, i)])
        else:
            row.append(' ')
    print(''.join(row))
